        ...
        if (method === 'GET') {
            ...
            if (lowAction === 'connected') {
                ...
                if (lowAction === 'pulseguide') {
                    ...
                    if (!coordTag) {
                        const candidates = MASTER_DICTIONARY.Telescope.coord;
                        coordTag = candidates.find(c => this.indiRef.properties.has(c.p));
                    }
                    ...
                    if (!setTag) {
                        const candidates = MASTER_DICTIONARY.Telescope.on_coord_set;
                        setTag = candidates.find(c => this.indiRef.properties.has(c.p));
                    }
                    const setProp = setTag?.p || 'ON_COORD_SET';
                    const slewEl = (typeof setTag?.e === 'object') ? setTag.e.slew : (setTag?.e || 'SLEW');

                    xml = `<newSwitchVector device='${dev}' name='${setProp}'><oneSwitch name='${slewEl}'>On</oneSwitch></newSwitchVector><newNumberVector device='${dev}' name='${p}'><oneNumber name='${raEl}'>${ra}</oneNumber><oneNumber name='${decEl}'>${dec}</oneNumber></newNumberVector>`;
                }
