            ...
            <><div className="space-y-1"><div className="flex justify-between items-center mb-2"><h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Device Controls</h3>{isINDI && (<span className="text-[10px] text-slate-500 font-mono">{t('controlPanel.receivedPackets', { count: indiMessageCount })}</span>)}</div>
                    {useDynamicDeviceList ? (
                        (indiDevices || []).length === 0 ? (
                            ...
                            <div className="space-y-2">{(indiDevices || []).map((dev) => {
                                const activeCam = AstroService.getActiveCamera();
                                const activeFoc = AstroService.getActiveFocuser();
                                const isCamera = dev.type === 'Camera';
                                const isFocuser = dev.type === 'Focuser';
                                return (
                                <div key={dev.name} className="flex items-center justify-between p-2 bg-slate-800/60 rounded border border-slate-700 hover:border-slate-600">
                                    <div className="flex items-center gap-2 overflow-hidden">
                                        {isCamera && (
                                            <input type="radio" name="main_camera_select" checked={dev.name === activeCam} onChange={() => AstroService.setActiveCamera(dev.name)} className="text-red-500 focus:ring-red-500 bg-slate-700 border-slate-600 cursor-pointer shrink-0" title="メインカメラとして選択" />
                                        )}
                                        {isFocuser && (
                                            <input type="radio" name="main_focuser_select" checked={dev.name === activeFoc} onChange={() => AstroService.setActiveFocuser(dev.name)} className="text-red-500 focus:ring-red-500 bg-slate-700 border-slate-600 cursor-pointer shrink-0" title="メインフォーカサーとして選択" />
                                        )}
                                    ...
                                    <div className="flex items-center gap-2 shrink-0">
                                        ...
                                        {!isAlpaca && (<button className="p-1 hover:bg-slate-600 rounded text-red-400 hover:text-red-200" title={t('tooltips.deviceSettings')} onClick={() => onOpenDeviceSettings(dev.type || 'Camera', dev.name)}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3"></circle><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2-2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 0 2.83 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"></path></svg></button>)}
                                    </div>
                                </div>
                                );
                            })}<div className="pt-2 text-center"><button onClick={() => AstroService.refreshIndiDevices()} className="text-[10px] text-slate-500 hover:text-slate-300 underline">{t('controlPanel.refreshDeviceList')}</button></div></div>
                        )
                    ) : (
