...
export const DeviceSettingsModal: React.FC<DeviceSettingsModalProps> = ({ 
    ...
    useEffect(() => {
        if (isOpen && deviceName) {
            ...
            updateProps();
            const interval = setInterval(updateProps, 200); // Polling faster (200ms) for better responsiveness
    ...
    const handleApply = (vector: INDIVector, values: Record<string, any>) => {
        if (deviceName) {
            ...
            // Clear editing state for these fields on submit
            ...
            <div key={vector.name} className="bg-slate-800/50 p-3 rounded border border-slate-700 space-y-2">
                                ...
                                <div key={el.name} className="flex flex-col gap-1">
                                    ...
                                    <div className="flex gap-2">
                                        <input 
                                            ...
                                            onKeyDown={(e) => {
                                                if (e.key === 'Enter') {
                                                    handleApply(vector, { [el.name]: (e.target as HTMLInputElement).value });
                                                }
                                            }}
                                            disabled={isRO}
                                        />
                ...
                {vector.type === 'Text' && (
                 ...
                 {vector.type === 'Light' && (
                     <div className="flex flex-wrap gap-2">
                        {Array.from(vector.elements.values()).map(el => (
                             <div key={el.name} className={`px-2 py-1 rounded text-xs border ${el.value === 'Ok' ? 'bg-green-900 border-green-700 text-green-200' : el.value === 'Alert' ? 'bg-red-900 border-red-700 text-red-200' : 'bg-slate-700 border-slate-600 text-slate-400'}`}>
                                 {el.label}: {el.value}
                             </div>
                        ))}
                     </div>
                        ...
                        <div className="flex-1 flex flex-col gap-3 min-h-0">
                            ...
                            <div className="flex-1 flex flex-col min-h-0">
                                <LinkedMiniView 
                                    ...
                                    latestImageMetadata={latestImageMetadata} // Passed metadata
                                    latestImageFormat={latestImageFormat}     // Passed format
