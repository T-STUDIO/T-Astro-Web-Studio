...
export const DeviceSettingsModalSimulator: React.FC<DeviceSettingsModalProps> = ({ 
    isOpen, 
    deviceType, 
    deviceName, 
    onClose,
                            ...
                            return (
                                <div key={el.name} className="flex flex-col gap-1">
                                    <div className="flex justify-between text-xs text-slate-400">
                                        <span>{el.label}</span>
                                        <span className="font-mono">{typeof el.value === 'number' ? el.value.toFixed(2) : el.value}</span>
                                    </div>
                                        ...
                                        {!isRO && (
                                            <button 
                                                onClick={() => {
                                                    ...
                                                    handleApply(vector, { [el.name]: Number(currentVal) });
                                                }}
                ...
                {vector.type === 'Switch' && (
                    <div className="flex flex-wrap gap-2">
                        {Array.from(vector.elements.values()).map(el => (
                            <button
                                key={el.name}
                                onClick={() => !isRO && handleApply(vector, { [el.name]: true })}
                                disabled={isRO}
                                className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${el.value === 'On' || el.value === true ? 'bg-green-600 text-white' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}
                            >
                                {el.label}
                            </button>
                        ))}
                        ...
                        {Array.from(vector.elements.values()).map(el => {
                            ...
                            return (
                                <div key={el.name} className="flex flex-col gap-1">
                                    <div className="flex justify-between text-xs text-slate-400">
                                        <span>{el.label}</span>
                                        <span className="font-mono truncate max-w-[150px]">{String(el.value || '')}</span>
                                    </div>
                                    <div className="flex gap-2">
                                        <input 
                                            type="text" 
