...
export const ImagingView: React.FC<ImagingViewProps> = ({ 
  ...
  const calculateCameraFov = (): number => {
    try {
      ...
      if (focalLength === 0 && typeof window !== 'undefined') {
        const savedFocal = localStorage.getItem('planetarium_manual_focal_length');
        if (savedFocal) focalLength = parseFloat(savedFocal) || 0;
      }

      if (focalLength === 0) focalLength = 180;

      let width = params.width > 0 ? params.width : 4656;
      let height = params.height > 0 ? params.height : 3520;
      let pixelSize = params.pixelSize > 0 ? params.pixelSize : 3.8;
      ...
      if (activeCam && typeof window !== 'undefined') {
        try {
          ...
          if (savedParams) {
            ...
            if (parsed[activeCam]) {
              const cp = parsed[activeCam];
              if (cp.width) width = cp.width;
              if (cp.height) height = cp.height;
              if (cp.pixelSize) pixelSize = cp.pixelSize;
            }
          }
        } catch (e) {}
      }

      const sw = width * (pixelSize / 1000.0);
      const sh = height * (pixelSize / 1000.0);
  ...
  const cachedRawChannelsRef = useRef<{ ch0: Float32Array; ch1: Float32Array | null; ch2: Float32Array | null; width: number; height: number; bpp: number } | null>(null);
  ...
  const resetImagingData = useCallback(() => {
    ...
    cachedRawChannelsRef.current = null;
  ...
  const drawOverlays = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    const applyLutAndDraw = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number, overrideRawFits?: any) => {
        ...
        let currentCache = cachedRawChannelsRef.current;
        if (overrideRawFits && overrideRawFits.ch0 && overrideRawFits.ch0.length === len) {
            cachedRawChannelsRef.current = {
                ch0: overrideRawFits.ch0,
                ch1: overrideRawFits.ch1,
                ch2: overrideRawFits.ch2,
                width,
                height,
                bpp: overrideRawFits.bpp || 16
            };
            currentCache = cachedRawChannelsRef.current;
        }

        if (!currentCache) {
            let bpp = 8;
            if (localFitsHeaders && (localFitsHeaders.BITPIX || localFitsHeaders.BPP || localFitsHeaders.bitpix)) {
                const bp = Math.abs(parseInt(localFitsHeaders.BITPIX || localFitsHeaders.BPP || localFitsHeaders.bitpix, 10));
                if (bp === 8 || bp === 16 || bp === 32) bpp = bp;
            } else if (externalMetadata && (externalMetadata.BITPIX || externalMetadata.BPP || externalMetadata.bitpix || externalMetadata.bpp)) {
                const bp = Math.abs(parseInt(externalMetadata.BITPIX || externalMetadata.BPP || externalMetadata.bitpix || externalMetadata.bpp, 10));
            ...
            const ch2 = new Float32Array(len);
            for (let i = 0; i < len; i++) {
            ...
            cachedRawChannelsRef.current = { ch0, ch1, ch2, width, height, bpp };
            currentCache = cachedRawChannelsRef.current;
        }

        const bpp = currentCache.bpp;
        const maxVal = bpp <= 8 ? 255 : 65535;
        const ch0 = currentCache.ch0;
        const ch1 = currentCache.ch1;
        const ch2 = currentCache.ch2;
  ...
  const renderRawFrame = useCallback((sourceBuffer: Uint8ClampedArray, width: number, height: number) => {
      ...
      const rawFitsData = externalMetadata?.rawFitsData;
      if (rawFitsData && rawFitsData.ch0 && rawFitsData.ch0.length === width * height) {
          cachedRawChannelsRef.current = {
              ch0: rawFitsData.ch0,
              ch1: rawFitsData.ch1,
              ch2: rawFitsData.ch2,
          ...
          const ch2 = new Float32Array(len);
          for (let i = 0; i < len; i++) {
              ch0[i] = swapRB ? sourceBuffer[i * 4 + 2] : sourceBuffer[i * 4];
              ch1[i] = sourceBuffer[i * 4 + 1];
              ch2[i] = swapRB ? sourceBuffer[i * 4] : sourceBuffer[i * 4 + 2];
          }
          cachedRawChannelsRef.current = {
              ch0,
              ch1,
  ...
  }, [applyLutAndDraw, fitImageToScreen, externalMetadata, swapRB]);
  ...
  const renderMjpegFrame = useCallback(async (blobOrUrl: Blob | string) => {
      ...
      const drawToCanvas = (drawable: CanvasImageSource, w: number, h: number) => {
          ...
          if (tempCtx) {
              tempCtx.drawImage(drawable, 0, 0, w, h);
              ...
              const ch2 = new Float32Array(len);
              for (let i = 0; i < len; i++) {
                  ch0[i] = swapRB ? data[i * 4 + 2] : data[i * 4];
                  ch1[i] = data[i * 4 + 1];
                  ch2[i] = swapRB ? data[i * 4] : data[i * 4 + 2];
              }
              cachedRawChannelsRef.current = {
                  ch0,
                  ch1,
  ...
  }, [fitImageToScreen, applyLutAndDraw, swapRB]);
  ...
  const processImage = useCallback(() => {
      ...
      const rawFitsData = localFitsHeaders?.rawFitsData;
      if (rawFitsData && rawFitsData.ch0 && rawFitsData.ch0.length === img.naturalWidth * img.naturalHeight) {
          cachedRawChannelsRef.current = {
              ch0: rawFitsData.ch0,
              ch1: rawFitsData.ch1,
              ch2: rawFitsData.ch2,
  ...
  }, [applyLutAndDraw, localFitsHeaders, swapRB]);
  ...
  useEffect(() => {
      ...
      const cache = cachedRawChannelsRef.current;
      if (cache) {
          applyLutAndDraw(ctx, cache.width, cache.height, externalMetadata?.rawFitsData || localFitsHeaders?.rawFitsData);
      } else if (lastRawFrameRef.current) {
  ...
  const handleAutoStretch = () => {
      ...
      if (cache) {
          bpp = cache.bpp;
      } else if (rawFitsData && rawFitsData.bpp) {
          bpp = rawFitsData.bpp;
      } else if (localFitsHeaders && (localFitsHeaders.BITPIX || localFitsHeaders.BPP || localFitsHeaders.bitpix)) {
          const bp = Math.abs(parseInt(localFitsHeaders.BITPIX || localFitsHeaders.BPP || localFitsHeaders.bitpix, 10));
          if (bp === 8 || bp === 16 || bp === 32) bpp = bp;
      ...
      samples.sort((a, b) => a - b);
      const p01 = (samples[Math.floor(samples.length * 0.01)] || 0) * (maxVal / 255);
      const p99 = (samples[Math.floor(samples.length * 0.99)] || 255) * (maxVal / 255);
      setBlackPoint(Math.max(0, Math.floor(p01)));
      setWhitePoint(Math.min(maxVal, Math.ceil(p99)));
      setMidPoint(1.0);
  };
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (/\.fits?$/i.test(file.name)) {
          reader.onload = async (ev) => { if (ev.target?.result instanceof ArrayBuffer) { const res = await AstroService.rawFitsToDisplay(ev.target.result, 'fits', debayerPattern); if (res.url) { setLoadedImage(res.url); setLoadedImageName(file.name); setLocalFitsHeaders(res.headers); } } };
  ...
  const cameras = devices.filter((d: any) => d.type === 'Camera' || (d.name && (d.name.toLowerCase().includes('camera') || d.name.toLowerCase().includes('ccd') || d.name.toLowerCase().includes('cam') || d.name.toLowerCase().includes('webcam'))));
        ...
        {!isMini && cameras.length > 1 && (
            <div className="absolute top-3 left-3 z-40 pointer-events-auto" {...stopProp}>
                <button
                    ...
                    title={t('common.switchCamera') || 'カメラ切替'}
                >
                    ...
                    <span>{t('common.switchCamera') || 'カメラ切替'}</span>
                </button>
            </div>
        )}
                    ...
                    <div className="relative">
                        ...
                        {showSaveMenu && (
                            <div className="absolute bottom-full right-0 mb-2 bg-slate-800 rounded-lg border border-slate-600 shadow-xl overflow-hidden flex flex-col w-28 md:w-32">
                                <button className="px-3 py-2 text-xs text-left hover:bg-slate-700 text-slate-200" onClick={async () => { if (canvasRef.current) { const mountPos = AstroService.getTelescopePosition(); const blob = exportJPEG(canvasRef.current, exifBinary, solvedCalibration, location, mountPos); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'capture.jpg'; a.click(); } setShowSaveMenu(false); }}>JPEG</button>
                                <button className="px-3 py-2 text-xs text-left hover:bg-slate-700 text-slate-200" onClick={async () => { if (canvasRef.current) { const mountPos = AstroService.getTelescopePosition(); const blob = await exportFITS(canvasRef.current, solvedCalibration, location, mountPos); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'capture.fits'; a.click(); } setShowSaveMenu(false); }}>FITS</button>
                                <button className="px-3 py-2 text-xs text-left hover:bg-slate-700 text-slate-200" onClick={async () => { if (canvasRef.current) { const mountPos = AstroService.getTelescopePosition(); const blob = await exportTIFF(canvasRef.current, solvedCalibration, location, mountPos); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'capture.tif'; a.click(); } setShowSaveMenu(false); }}>TIFF</button>
                                <button className="px-3 py-2 text-xs text-left hover:bg-slate-700 text-slate-200" onClick={async () => { if (canvasRef.current) { const mountPos = AstroService.getTelescopePosition(); const blob = await exportPNG(canvasRef.current, solvedCalibration, location, mountPos); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'capture.png'; a.click(); } setShowSaveMenu(false); }}>PNG</button>
                            </div>
                        )}
                    </div>
                )}
