...
type TSConectTab = 'connection' | 'location' | 'solver' | 'system';

interface TSConectProps {
    onClose: () => void;
...
export const TSConect: React.FC<TSConectProps> = (props) => {
    const { t } = useTranslation();
    const [activeTab, setActiveTab] = useState<TSConectTab>('connection');
    ...
    const TabButton = ({ id, label }: { id: TSConectTab, label: string }) => (
        <button 
        ...
        <div className="fixed inset-0 top-16 bg-[#020617] z-[45] flex flex-col overflow-hidden animate-fadeIn select-none">
            ...
            <div className="flex bg-slate-900 shrink-0 border-b border-slate-800 shadow-lg items-center">
                ...
                <div className="flex items-center gap-2 px-3 border-l border-slate-800 h-full">
                    ...
                    <button onClick={props.onClose} className="p-2 text-slate-500 hover:text-white transition-colors" title="Close">
                                        ...
                                        {props.connectionStatus !== 'Connected' ? (
                                            <div className="space-y-4">
                                                ...
                                                <div className="grid grid-cols-2 gap-4">
                                                    <LargeInput label={t('controlPanel.host')} value={props.connectionSettings.host} onChange={(v: string) => props.onSettingsChange({...props.connectionSettings, host: v})} />
                                                    <LargeInput label={t('controlPanel.port')} value={props.connectionSettings.port} type="number" onChange={(v: string) => props.onSettingsChange({...props.connectionSettings, port: Number(v)})} />
                                                </div>
                            ...
                            <div className="max-w-2xl mx-auto space-y-6">
                                <div className="grid grid-cols-1 gap-4 p-6 bg-slate-900/60 rounded-2xl border border-slate-800 shadow-md">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <SexagesimalInput label={t('controlPanel.location.latitude')} value={props.location?.latitude || 0} onChange={props.onUpdateLatitude} />
                                        <SexagesimalInput label={t('controlPanel.location.longitude')} value={props.location?.longitude || 0} onChange={props.onUpdateLongitude} />
                                    </div>
                                    <LargeInput label={t('controlPanel.location.elevation')} value={props.location?.elevation || 0} type="number" onChange={(v: string) => props.onUpdateElevation(Number(v))} />
                                    ...
                                    <div className="space-y-2 mt-2">
                                        <Button onClick={props.onSendLocationToMount} disabled={!isConnected} className="w-full py-5 text-xl font-black bg-blue-700 hover:bg-blue-600 rounded-xl shadow-lg border-b-4 border-blue-900 active:border-b-0 active:translate-y-1 transition-all">
                                ...
                                <div className="p-6 bg-slate-900/60 rounded-2xl border border-slate-800 flex items-center justify-between shadow-md">
                                    <div className="space-y-1">
                                        ...
                                        <div className="text-4xl font-mono font-black text-red-500 tabular-nums">{props.localTime.toLocaleTimeString()}</div>
                                    </div>
                            ...
                            <div className="max-w-2xl mx-auto space-y-6">
                                <div className="p-6 bg-slate-900/60 rounded-2xl border border-slate-800 shadow-md space-y-6">
                                    <div className="flex bg-slate-800 p-1 rounded-xl border border-slate-700">
                                        <button onClick={() => props.onSetPlateSolverType('Remote')} className={`flex-1 py-3 text-lg font-black rounded-lg transition-all ${props.plateSolverType === 'Remote' ? 'bg-red-700 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'}`}>Remote (Nova)</button>
                                        <button onClick={() => props.onSetPlateSolverType('Local')} className={`flex-1 py-3 text-lg font-black rounded-lg transition-all ${props.plateSolverType === 'Local' ? 'bg-red-700 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'}`}>Local API</button>
                                    </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="p-6 bg-slate-900/60 rounded-2xl border border-slate-800 shadow-md space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 gap-3">
                                        ...
                                        <Button onClick={props.onSaveToDisk} variant="secondary" className="py-3 text-sm font-black bg-slate-800">
                                            <SaveIcon className="w-5 h-5" /> {t('controlPanel.saveToDevice')}
                                        </Button>
                                        <Button onClick={() => fileInputRef.current?.click()} variant="secondary" className="py-3 text-sm font-black bg-slate-800">
                                ...
                                <div className="p-6 bg-slate-900/60 rounded-2xl border border-slate-800 shadow-md space-y-4">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <Button onClick={props.onConnectSamp} disabled={props.sampStatus === 'Connecting'} className="w-full py-3 text-sm font-black bg-blue-700">
