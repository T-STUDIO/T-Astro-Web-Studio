...
export class BlobTransportService {
    ...
    public processIncomingChunk(chunk: Uint8Array) {
        // メインの通信ループを妨げないよう、完全に非同期なバックグラウンドタスクとして蓄積・パース処理を実行する
        setTimeout(async () => {
    ...
    public processIncomingString(text: string) {
    }
    public connect(settings: any) {
        return connectBlob(settings);
    }
    public disconnect() {
        return disconnectBlob();
    }
    public sendRaw(xml: string) {
        sendRaw(xml);
    }
    public enableBlobForDevice(devName: string) {
    }
    public setImageReceivedCallback(cb: any) {
        setImageReceivedCallbackBlob(cb);
    }
    public clearBuffer() {
        recvChunks = [];
        recvLength = 0;
...
export const setImageReceivedCallbackBlob = (cb: typeof onImageReceived) => onImageReceived = cb;
...
const getCameraParams = (devName: string) => {
    let driverParams: any = null;
    let numericValueFn: any = null;
    if (typeof window !== 'undefined') {
        if ((window as any).__getActiveCameraParams) {
            driverParams = (window as any).__getActiveCameraParams();
        }
        if ((window as any).__getNumericValue) {
            numericValueFn = (window as any).__getNumericValue;
        }
    }

    if (driverParams && (driverParams.width > 0 || driverParams.height > 0)) {
        return driverParams;
    }

    const dev = discoveredIndiDevices.get(devName);
    if (!dev) {
        let width = (numericValueFn ? (numericValueFn(devName, 'CCD_FRAME', 'WIDTH') || numericValueFn(devName, 'CCD_CFA', 'WIDTH')) : 0) || 0;
        let height = (numericValueFn ? (numericValueFn(devName, 'CCD_FRAME', 'HEIGHT') || numericValueFn(devName, 'CCD_CFA', 'HEIGHT')) : 0) || 0;
        let pixelSize = (numericValueFn ? numericValueFn(devName, 'CCD_INFO', 'PixelSize') : 0) || 0;
        let bpp = (numericValueFn ? numericValueFn(devName, 'CCD_INFO', 'BitsPerPixel') : 0) || 8;
        return { width, height, bpp, pixelSize, format: driverParams?.format || '' };
    }
...
export const connectBlob = async (settings: ConnectionSettings): Promise<boolean> => {
...
export const disconnectBlob = async () => {
    if (currentSettings.driver === 'Simulator') {
