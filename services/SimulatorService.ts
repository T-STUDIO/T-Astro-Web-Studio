...
export function getAladinImageUrl(position: TelescopePosition, settings: SimulatorSettings): string {
  ...
  const url = `https://aladin.u-strasbg.fr/AladinLite/export/nph-export.cgi?ra=${ra}&dec=${dec}&fov=${fovDeg}&width=${width}&height=${height}&format=jpg&survey=P%2FDSS2%2Fcolor`;
  console.log(`[Simulator] Generated Aladin URL: ${url}`);
  return url;
}
